	...
	initComponent : function(){
		...
		this.insert(3,new Ext.Panel({
			...
			tpl:'<tpl if="status_history.length">'+
				...
				'<td>{user_name}</div></td>'+
				'<td>{ctime}</td>'+
				'<td><tpl if="notified&gt;0"><div class="go-grid-icon btn-ok"><a href="go:showMessage( \'{[Ext.util.Format.htmlEncode(values.notification_email)]}\')"><i class="icon">mail</i></a></div></tpl></td>'+
				'</tr>'+
				'</tpl>'+
				'</table></tpl>'
		}));
